...
type Screen = 'HOME' | 'PATIENT_INTAKE' | 'PATIENT_SELECT' | 'PATIENT_HISTORY' | 'DISCLAIMER' | 'GAME_INTRO' | 'GAME' | 'MEMORY_INTRO' | 'MEMORY' | 'QA_INTRO' | 'QA_LOADING' | 'QA' | 'RESULTS' | 'DETAILED_REPORT' | 'TREMOR_TEST' | 'TREMOR_REPORT' | 'SPIRAL_TEST' | 'SPIRAL_REPORT' | 'VOICE_TEST' | 'VOICE_REPORT';
    ...
    return (
        <div className="relative flex justify-center items-center py-4">
            <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="overflow-visible">
                ...
                <polygon points={points} className="fill-indigo-500/20 dark:fill-indigo-400/20 stroke-indigo-500 dark:stroke-indigo-400 drop-shadow-lg pointer-events-none" strokeWidth="3" />
                {data.map((d, i) => {
                    const { x, y } = getCoordinates(i, d.value, d.fullMark);
                    const isHovered = hoveredIndex === i;
                    return (
                        <g key={i}>
                            ...
                            <circle 
                                ...
                                className={`fill-indigo-600 dark:fill-indigo-400 stroke-white dark:stroke-slate-800 stroke-[3px] transition-all pointer-events-none`} 
                            />
                            {isHovered && (
                                <g className="pointer-events-none drop-shadow-lg z-50">
                                    <text x={x} y={y - 14} textAnchor="middle" fontSize="16" fontWeight="900" 
                                          className="fill-white drop-shadow-md stroke-slate-400/20 stroke-1 dark:stroke-slate-900/50">
                                        {d.value.toFixed(0)}
                                    </text>
                                </g>
                            )}
                ...
                {data.map((d, i) => {
                    ...
                    return (
                        <text key={i} x={x} y={y} textAnchor="middle" dominantBaseline="middle" className="text-[10px] font-bold fill-slate-500 dark:fill-slate-400 uppercase tracking-widest pointer-events-none">
                            {d.label}
                        </text>
                    );
                })}
...
export default function AssessmentPlatform() {
    ...
    const [graphMetric, setGraphMetric] = useState<'GCS' | 'RMS' | 'SPIRAL' | 'VOICE'>('GCS');
    ...
    const [historyFilter, setHistoryFilter] = useState<'ALL' | 'COGNITIVE' | 'MOTOR' | 'SPIRAL' | 'VOICE'>('ALL');

    // --- EWSS Computation ---
    const computeEWSS = (history: DB.AnyRecord[]) => {
        if (!history || history.length === 0) return null;

        // Find the absolute latest test date to define the current clinical "session"
        const latestDate = Math.max(...history.map(r => r.date));
        
        // Group by 'session': only include tests conducted within 24 hours of the most recent test
        const sessionHistory = history.filter(r => (latestDate - r.date) <= 24 * 60 * 60 * 1000);

        const latestSpiral = sessionHistory.find(r => r.type === 'SPIRAL') as DB.SpiralRecord | undefined;
        const latestTremor = sessionHistory.find(r => r.type === 'MOTOR') as DB.TremorRecord | undefined;
        const latestCognitive = sessionHistory.find(r => r.type === 'COGNITIVE') as DB.AssessmentRecord | undefined;
        const latestVoice = sessionHistory.find(r => r.type === 'VOICE') as DB.VoiceRecord | undefined;

        if (!latestSpiral && !latestTremor && !latestCognitive && !latestVoice) return null;

        // Normalization to [0, 1]
        const spiralScore = latestSpiral ? latestSpiral.score / 100 : 0; // Already 0-100 probability
        const voiceScore = latestVoice ? latestVoice.score / 100 : 0; // 0-100 probability
        ...
        if (latestCognitive) {
            // Inverse scale: score matches linear missing
            // 100 GCS = 0% risk, 0 GCS = 100% risk.
            cognitiveScore = Math.max(0, Math.min(1.0, (100 - latestCognitive.gcs) / 100));
        }
        ...
        if (latestSpiral) { totalWeight += 0.3; weightedSum += spiralScore * 0.3; }
        if (latestVoice) { totalWeight += 0.3; weightedSum += voiceScore * 0.3; }
        if (latestTremor) { totalWeight += 0.2; weightedSum += tremorScore * 0.2; }
        if (latestCognitive) { totalWeight += 0.2; weightedSum += cognitiveScore * 0.2; }
        ...
        // Raw un-normalized metrics for display
        const spiralRaw = latestSpiral ? latestSpiral.score : 0;
        const voiceRaw = latestVoice ? latestVoice.score : 0;
        const tremorRaw = latestTremor ? Math.max(latestTremor.left_hand.avg_rms, latestTremor.right_hand.avg_rms) : 0;
        const cognitiveRaw = latestCognitive ? latestCognitive.gcs : 0;
        ...
        return {
            ...
            colorTheme,
            badgeColor,
            hasFeatures: { spiral: !!latestSpiral, tremor: !!latestTremor, cognitive: !!latestCognitive, voice: !!latestVoice },
            raw: { spiral: spiralRaw, tremor: tremorRaw, cognitive: cognitiveRaw, voice: voiceRaw }
        };
    };
    ...
    const handleDeleteAssessment = async (e: React.MouseEvent, recordId: string, type: 'COGNITIVE' | 'MOTOR' | 'SPIRAL' | 'VOICE') => {
        e.stopPropagation();
    ...
    const startMemoryRound = (level: number) => { 
        ...
        setUserSequence([]); 
        setShowingIdx(null); 
        const newSeq = Array.from({ length: level }, () => Math.floor(Math.random() * DB.CONFIG.MEM_GRID_SIZE)); 
        setSequence(newSeq); 
        let i = 0; 
        if (memoryIntervalRef.current) clearInterval(memoryIntervalRef.current);
        memoryIntervalRef.current = window.setInterval(() => { 
            setShowingIdx(newSeq[i]); 
            setTimeout(() => setShowingIdx(null), DB.CONFIG.MEM_SHOW_TIME - 200); 
            i++; 
            if (i >= newSeq.length) { 
                if (memoryIntervalRef.current) clearInterval(memoryIntervalRef.current); 
                setTimeout(() => { 
                ...
                }, DB.CONFIG.MEM_INTER_STIM_TIME); 
            } 
        }, DB.CONFIG.MEM_SHOW_TIME); 
    };
                                ...
                                {!activePatient ? (
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <button onClick={() => setCurrentScreen('SPIRAL_TEST')} className="w-full bg-white border border-slate-200 hover:border-indigo-300 hover:bg-slate-50 text-slate-800 py-6 rounded-xl font-bold text-lg transition-all shadow-sm flex items-center justify-between px-8 group">
                                            ...
                                            <ChevronRight className="w-6 h-6 text-slate-300 group-hover:text-fuchsia-500 group-hover:translate-x-1 transition-all" />
                                        </button>

                                        <button onClick={() => setCurrentScreen('VOICE_TEST')} className="w-full bg-white border border-slate-200 hover:border-indigo-300 hover:bg-slate-50 text-slate-800 py-6 rounded-xl font-bold text-lg transition-all shadow-sm flex items-center justify-between px-8 group">
                                            <div className="flex items-center gap-4">
                                                <div className="p-3 bg-indigo-100 rounded-lg"><Mic className="w-8 h-8 text-indigo-600" /></div>
                                                <div className="text-left">
                                                    <div className="text-lg font-bold text-slate-800">Speech & Voice Analysis</div>
                                                    <div className="text-sm text-slate-500">Acoustic MFCC Parkinson's Detection</div>
                                                </div>
                                            </div>
                                            <ChevronRight className="w-6 h-6 text-slate-300 group-hover:text-indigo-500 group-hover:translate-x-1 transition-all" />
                                        </button>
    ...
    if (currentScreen === 'PATIENT_HISTORY') {
        ...
        const graphData = patientHistory
            .filter(r => {
                ...
                if (graphMetric === 'VOICE') return r.type === 'VOICE';
                return r.type === 'SPIRAL';
            })
            .sort((a, b) => a.date - b.date)
            .map(r => {
                if (graphMetric === 'GCS') return r;
                if (graphMetric === 'VOICE') {
                    const vr = r as DB.VoiceRecord;
                    return { ...vr, voiceScore: vr.score };
                }
                ...
                <div className="max-w-5xl mx-auto">
                    <div className="flex justify-between items-center mb-8">
                        ...
                        <div className={`flex gap-2 p-1 rounded-xl shadow-sm border ${bgCard}`}>
                            {(['ALL', 'COGNITIVE', 'MOTOR', 'SPIRAL', 'VOICE'] as const).map(f => (
                            ...
                            {ewssMetrics && (
                                <div className={`px-5 py-4 rounded-2xl border-2 ${ewssMetrics.colorTheme} flex items-center gap-4 md:gap-6 flex-wrap md:flex-nowrap w-full md:w-auto`}>
                                    ...
                                    <div className="flex gap-4 text-xs font-mono text-center">
                                        <div>
                                            <div className="font-bold opacity-70 uppercase text-[10px]">Voice</div>
                                            <div className="mt-1">{ewssMetrics.hasFeatures.voice ? ewssMetrics.raw.voice.toFixed(0) : '--'}</div>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <div className="mt-1">{ewssMetrics.hasFeatures.tremor ? ewssMetrics.raw.tremor.toFixed(2) : '--'}</div>
                                        </div>
                                        <div>
                    ...
                    <div className={`p-6 rounded-3xl shadow-sm border mb-8 ${bgCard}`}>
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <div className={`flex p-1 rounded-lg border ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-slate-100 border-slate-200'}`}>
                                ...
                                <button
                                    onClick={() => setGraphMetric('VOICE')}
                                    className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${graphMetric === 'VOICE' ? 'bg-white shadow text-indigo-600 dark:bg-slate-700 dark:text-white' : 'text-slate-400'}`}
                                >
                                    Voice (PD%)
                                </button>
                            </div>
                        ...
                        <div className="h-64">
                            <HistoryChart
                                data={graphData}
                                dataKey={graphMetric === 'GCS' ? 'gcs' : graphMetric === 'SPIRAL' ? 'spiralScore' : graphMetric === 'VOICE' ? 'voiceScore' : 'rms'}
                                color={graphMetric === 'GCS' ? (isDark ? "#818cf8" : "#4f46e5") : graphMetric === 'SPIRAL' ? (isDark ? "#e879f9" : "#c026d3") : graphMetric === 'VOICE' ? (isDark ? "#60a5fa" : "#3b82f6") : "#f59e0b"}
                                label={graphMetric === 'GCS' ? "Global Cognitive Score" : graphMetric === 'SPIRAL' ? "PD Probability %" : graphMetric === 'VOICE' ? "Voice PD Probability %" : "Max Tremor RMS"}
                            />
                        </div>
                    </div>
                                ...
                                <div key={r.id} onClick={() => handleViewRecord(r)} className={`p-4 rounded-xl border transition-all cursor-pointer flex items-center justify-between group ${bgCard} ${isDark ? "hover:bg-slate-700/50 hover:border-indigo-500" : "hover:bg-slate-50 hover:border-indigo-200"}`}>
                                    <div className="flex items-center gap-4">
                                        <div className={`p-3 rounded-lg ${r.type === 'MOTOR' ? (isDark ? 'bg-amber-900/30 text-amber-400' : 'bg-amber-100 text-amber-600') : (isDark ? 'bg-indigo-900/30 text-indigo-400' : 'bg-indigo-100 text-indigo-600')}`}>
                                            {r.type === 'MOTOR' ? <Zap className="w-5 h-5" /> : r.type === 'VOICE' ? <Mic className="w-5 h-5" /> : <Brain className="w-5 h-5" />}
                                        </div>
                                        <div>
                                            <div className={`font-bold ${textMain}`}>{r.type === 'MOTOR' ? 'Psychomotor Assessment' : r.type === 'SPIRAL' ? 'Spiral Drawing Analysis' : r.type === 'VOICE' ? 'Speech & Voice Analysis' : 'Cognitive Assessment'}</div>
                                    ...
                                    <div className="flex items-center gap-8">
                                        <div className="text-right">
                                            ...
                                            <div className={`font-bold text-lg ${r.type === 'MOTOR' ? getScoreColor(Math.max((r as DB.TremorRecord).left_hand.severity_score, (r as DB.TremorRecord).right_hand.severity_score) * 10) :
                                                r.type === 'SPIRAL' ? ((r as DB.SpiralRecord).score > 50 ? 'text-rose-500' : 'text-emerald-500') : r.type === 'VOICE' ? ((r as DB.VoiceRecord).score > 50 ? 'text-rose-500' : 'text-emerald-500') :
                                                ...
                                                {r.type === 'MOTOR' ? Math.max((r as DB.TremorRecord).left_hand.severity_score, (r as DB.TremorRecord).right_hand.severity_score).toFixed(1) :
                                                    r.type === 'SPIRAL' ? `${(r as DB.SpiralRecord).score.toFixed(1)}%` : r.type === 'VOICE' ? `${(r as DB.VoiceRecord).score.toFixed(1)}%` :
    ...
    if (currentScreen === 'VOICE_REPORT') {
        return (
            <div className={`min-h-screen transition-colors ${bgMain}`}>
                <VoiceReport record={selectedRecord as DB.VoiceRecord} onBack={() => setCurrentScreen('PATIENT_HISTORY')} onDelete={(id) => handleDeleteAssessment({ stopPropagation: () => { } } as any, id, 'VOICE')} isDark={isDark} />
            </div>
        );
    }

    if (currentScreen === 'VOICE_TEST') {
        return (
            <div className={`min-h-screen transition-colors ${bgMain}`}>
                <VoiceTest patientId={activePatient?.id || 'guest'} onBack={() => {
                    if (activePatient) refreshHistory(activePatient.id);
                    setCurrentScreen('HOME');
                }} isDark={isDark} />
            </div>
