...
export interface VoiceRecord {
    ...
    riskLabel?: 'Low Risk' | 'Medium Risk' | 'High Risk';
    confidenceLevel?: 'High' | 'Low - Retest Suggested';
    audioUrl?: string; // Base64 data URL of the recorded audio
    prompt?: string;   // The reading prompt used
}

export type AnyRecord = (AssessmentRecord & { type: 'COGNITIVE' }) | (TremorRecord & { type: 'MOTOR' }) | (SpiralRecord & { type: 'SPIRAL' }) | (VoiceRecord & { type: 'VOICE' });
...
export const initDB = async (): Promise<boolean> => {
    try {
        ...
        const TABLES = ['patients', 'clinical_records', 'question_logs', 'tremor_records', 'spiral_records', 'voice_records'];
...
export const getVoiceHistory = async (patientId: string): Promise<VoiceRecord[]> => {
    try {
        const res = (alasql as any)('SELECT * FROM voice_records WHERE patient_id = ? ORDER BY date DESC', [patientId]);
        return res.map((r: any) => ({
            ...r,
            confidenceLevel: r.confidence,
            riskLabel: r.risk,
            audioUrl: r.audio_b64 || undefined,
...
export const getAllHistory = async (patientId: string): Promise<AnyRecord[]> => {
    ...
    const voice = await getVoiceHistory(patientId);
    ...
    const combined = [
        ...
        ...voice.map(r => ({ ...r, type: 'VOICE' as const }))
    ];
...
export const insertVoiceRecord = async (r: VoiceRecord) => {
    try {
        (alasql as any)('INSERT INTO voice_records (id, patient_id, date, score, confidence, risk, audio_b64, prompt) VALUES (?,?,?,?,?,?,?,?)',
            [r.id, r.patient_id, r.date, r.score, r.confidenceLevel || '', r.riskLabel || '', r.audioUrl || '', r.prompt || '']);
    } catch (e) {
        // console.error(e);
    }
};
...
export const deleteVoiceRecord = async (id: string) => {
    (alasql as any)('DELETE FROM voice_records WHERE id = ?', [id]);
};

export const deleteRecord = async (id: string, type: 'COGNITIVE' | 'MOTOR' | 'SPIRAL' | 'VOICE') => {
    if (type === 'COGNITIVE') {
...
export const deletePatient = async (patientId: string) => {
    ...
    (alasql as any)('DELETE FROM spiral_records WHERE patient_id = ?', [patientId]);
    (alasql as any)('DELETE FROM voice_records WHERE patient_id = ?', [patientId]);
};
