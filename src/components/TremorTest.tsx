...
export default function TremorTest({ patientId, onBack, isDark }: TremorTestProps) {
    ...
    const processData = (side: 'left' | 'right', rawData: any) => {
        ...
        setTarget(prev => {
            const updatedData = isRecording ? [...prev.data, newPoint] : [...prev.data, newPoint].slice(-100);
            return {
                ...prev,
                current: rawData,
                data: updatedData
    ...
    const calibrate = () => {
        if (wsLeft.current) wsLeft.current.send("CALIBRATE");
        if (wsRight.current) wsRight.current.send("CALIBRATE");
        setCalibrating(true);
        // "wait for 5 sec"
        setTimeout(() => {
