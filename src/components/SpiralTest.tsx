...
export default function SpiralTest({ patientId, onBack, isDark }: SpiralTestProps) {
    ...
    const handleSaveRecord = async () => {
        ...
        try {
            ...
            console.error("Error inserting spiral record:", error);
        }
        onBack();
    };
                ...
                <div className={`max-w-2xl w-full p-8 md:p-12 rounded-3xl border shadow-xl ${bgCard} relative overflow-hidden`}>
                    ...
                    <div className="grid md:grid-cols-2 gap-8 mb-8">
                        ...
                        <div className="flex flex-col items-center justify-center">
                            ...
                            {resultImageUrl ? (
                                <img src={resultImageUrl} alt="Spiral Drawing" className={`w-48 h-48 border-4 rounded-xl object-contain bg-white shadow-md ${isDark ? 'border-slate-700' : 'border-slate-200'}`} />
                            ) : null}
                        </div>
                    </div>
